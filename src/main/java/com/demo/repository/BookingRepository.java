...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    @Query(
            value = "select b.* from booking b join customer_slot c on b.id_index = c.id_index\n" +
                    "where c.id_c_slot = ?1 and c.id_area = ?2 and b.is_deleted = 0 and b.is_enabled = 1", nativeQuery = true
    )
    ...
    (value = "select b.* from booking b join payment_c pc on b.id_booking = pc.id_booking " +
            "where pc.id_payment = ?1 and b.is_deleted = 0 and b.is_enabled = 1", nativeQuery = true)
    Booking findBookingByIdPayment(String id_Payment);

    @Query
            (value = "select b.* from booking b  " +
                    "where b.id_customer = ?1 and b.is_deleted = 0 and b.is_enabled = 1", nativeQuery = true)
    List<Booking> findBookingByCustomer(String id_Customer);

    @Query(
            value = "select b.* from booking b join customer_slot c on b.id_index = c.id_index\n" +
                    "where c.id_area = ?1 and b.is_deleted = 0 and b.is_enabled = 1", nativeQuery = true
    )
    ...
    (
            value = "select b.* from booking b join customer_slot cs on b.id_index = cs.id_index " +
                    "where cs.id_area = ?1 and cs.id_c_slot = ?2 and b.id_customer = ?3 and b.is_deleted = 0 and b.is_enabled = 1", nativeQuery = true
    )
